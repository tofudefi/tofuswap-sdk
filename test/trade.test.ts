...
describe('Trade', () => {
  ...
  it('can be constructed with TRX as input', () => {
    const trade = new Trade(
      new Route([pair_wtrx_0], TRX),
      CurrencyAmount.trx(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(TRX)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with TRX as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_wtrx_0], TRX, token0),
  ...
  it('can be constructed with TRX as output', () => {
    const trade = new Trade(
      new Route([pair_wtrx_0], token0, TRX),
      CurrencyAmount.trx(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(TRX)
  })
  it('can be constructed with TRX as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_wtrx_0], token0, TRX),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for TRX currency input', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wtrx_0, pair_0_1, pair_0_3, pair_1_3],
        CurrencyAmount.trx(JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(TRX)
      expect(result[0].route.path).toEqual([WTRX[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(TRX)
      expect(result[1].route.path).toEqual([WTRX[ChainId.MAINNET], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for TRX currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wtrx_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WTRX[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(TRX)
